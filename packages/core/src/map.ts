...
import type { TrackingStatus, TreeMarker } from './domain';
...
// The grouping itself, beside the cell size and the zoom thresholds it reads.
//
// It lived in the mobile app until the map performance work, which is where it
// is used. It moved here so `pnpm test:map` can measure the real function
// rather than a copy of it: a performance figure produced by a paraphrase says
// nothing about the code that ships. It is pure -- markers in, markers or
// clusters out -- and depends on nothing but the two constants above, so it
// belongs on this side of the line anyway.
